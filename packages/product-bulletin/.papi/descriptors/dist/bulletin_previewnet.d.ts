...
export type Bulletin_previewnet = {
    descriptors: {
        pallets: PalletsTypedef;
        apis: IRuntimeCalls;
    } & Promise<any>;
    metadataTypes: Promise<Uint8Array>;
    extensions: Bulletin_previewnetExtensions;
    requiredExtensions: IRequiredExtensions;
    getMetadata: () => Promise<Uint8Array>;
    genesis: string | undefined;
};
declare const _allDescriptors: Bulletin_previewnet;
export default _allDescriptors;
export type Bulletin_previewnetApis = ApisFromDef<IRuntimeCalls>;
export type Bulletin_previewnetQueries = QueryFromPalletsDef<PalletsTypedef>;
export type Bulletin_previewnetCalls = TxFromPalletsDef<PalletsTypedef>;
export type Bulletin_previewnetEvents = EventsFromPalletsDef<PalletsTypedef>;
...
export type Bulletin_previewnetWhitelistEntry = PalletKey | `query.${NestedKey<AllInteractions['storage']>}` | `tx.${NestedKey<AllInteractions['tx']>}` | `event.${NestedKey<AllInteractions['events']>}` | `error.${NestedKey<AllInteractions['errors']>}` | `const.${NestedKey<AllInteractions['constants']>}` | `view.${NestedKey<AllInteractions['viewFns']>}` | `api.${NestedKey<AllInteractions['apis']>}`;
type PalletKey = `*.${({
