...
export type Bulletin_paseoDispatchError = Anonymize<I2no2ge71kivps>;
export type Bulletin_paseoExtensions = {
    "CheckSpecVersion": {
...
export type Bulletin_paseo = {
    descriptors: {
        pallets: PalletsTypedef;
        apis: IRuntimeCalls;
    } & Promise<any>;
    metadataTypes: Promise<Uint8Array>;
    extensions: Bulletin_paseoExtensions;
    requiredExtensions: IRequiredExtensions;
    getMetadata: () => Promise<Uint8Array>;
    genesis: string | undefined;
};
declare const _allDescriptors: Bulletin_paseo;
export default _allDescriptors;
export type Bulletin_paseoApis = ApisFromDef<IRuntimeCalls>;
export type Bulletin_paseoQueries = QueryFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoCalls = TxFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoEvents = EventsFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoErrors = ErrorsFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoConstants = ConstFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoViewFns = ViewFnsFromPalletsDef<PalletsTypedef>;
export type Bulletin_paseoCallData = Anonymize<I5h0sno917s28c> & {
    value: {
        type: string;
    };
};
...
export type Bulletin_paseoWhitelistEntry = PalletKey | `query.${NestedKey<AllInteractions['storage']>}` | `tx.${NestedKey<AllInteractions['tx']>}` | `event.${NestedKey<AllInteractions['events']>}` | `error.${NestedKey<AllInteractions['errors']>}` | `const.${NestedKey<AllInteractions['constants']>}` | `view.${NestedKey<AllInteractions['viewFns']>}` | `api.${NestedKey<AllInteractions['apis']>}`;
type PalletKey = `*.${({
