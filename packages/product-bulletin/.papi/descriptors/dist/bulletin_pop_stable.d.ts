...
export type Bulletin_pop_stableDispatchError = Anonymize<I2no2ge71kivps>;
export type Bulletin_pop_stableExtensions = {
    "CheckSpecVersion": {
...
export type Bulletin_pop_stable = {
    descriptors: {
        pallets: PalletsTypedef;
        apis: IRuntimeCalls;
    } & Promise<any>;
    metadataTypes: Promise<Uint8Array>;
    extensions: Bulletin_pop_stableExtensions;
    requiredExtensions: IRequiredExtensions;
    getMetadata: () => Promise<Uint8Array>;
    genesis: string | undefined;
};
declare const _allDescriptors: Bulletin_pop_stable;
export default _allDescriptors;
export type Bulletin_pop_stableApis = ApisFromDef<IRuntimeCalls>;
export type Bulletin_pop_stableQueries = QueryFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableCalls = TxFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableEvents = EventsFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableErrors = ErrorsFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableConstants = ConstFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableViewFns = ViewFnsFromPalletsDef<PalletsTypedef>;
export type Bulletin_pop_stableCallData = Anonymize<I5h0sno917s28c> & {
    value: {
        type: string;
    };
};
...
export type Bulletin_pop_stableWhitelistEntry = PalletKey | `query.${NestedKey<AllInteractions['storage']>}` | `tx.${NestedKey<AllInteractions['tx']>}` | `event.${NestedKey<AllInteractions['events']>}` | `error.${NestedKey<AllInteractions['errors']>}` | `const.${NestedKey<AllInteractions['constants']>}` | `view.${NestedKey<AllInteractions['viewFns']>}` | `api.${NestedKey<AllInteractions['apis']>}`;
type PalletKey = `*.${({
