...
describe('createAttestationService', () => {
  ...
  describe('registerLitePerson', () => {
    function createMockApi(signSubmitAndWatch: ReturnType<typeof vi.fn>) {
      return {
        query: { PeopleLite: {} },
        tx: {
          PeopleLite: {
            attest: vi.fn(() => ({ signSubmitAndWatch })),
          },
        },
      };
    }

    function mockSuccessObservable() {
      return vi.fn(() => ({
        subscribe(observer: { next?: (event: any) => void; error?: (err: any) => void }) {
          queueMicrotask(() => observer.next?.({ type: 'finalized', ok: true }));
          return { unsubscribe: vi.fn() };
        },
      }));
    }

    function mockErrorObservable(error?: unknown) {
      return vi.fn(() => ({
        subscribe(observer: { next?: (event: any) => void; error?: (err: any) => void }) {
          queueMicrotask(() => observer.error?.(error ?? new Error('Stale')));
          return { unsubscribe: vi.fn() };
        },
      }));
    }
    ...
    it('succeeds on first attempt without retry', async () => {
      const signSubmitAndWatch = mockSuccessObservable();
      const mockApi = createMockApi(signSubmitAndWatch);
      const lazyClient = { getClient: () => ({ getUnsafeApi: () => mockApi }) } as any;

      const service = createAttestationService(lazyClient);
      const result = await service.registerLitePerson('guest.1234', createMockAccount(), createMockAccount());

      expect(result.isOk()).toBe(true);
      expect(signSubmitAndWatch).toHaveBeenCalledTimes(1);
    });

    it('retries signSubmitAndWatch on error and succeeds', async () => {
      let callCount = 0;
      const signSubmitAndWatch = vi.fn(() => ({
        subscribe(observer: { next?: (event: any) => void; error?: (err: any) => void }) {
          ...
          queueMicrotask(() => {
            if (count === 1) {
              observer.error?.(new Error('Stale'));
            } else {
              observer.next?.({ type: 'finalized', ok: true });
            }
          });
          return { unsubscribe: vi.fn() };
        },
      ...
      expect(result.isOk()).toBe(true);
      expect(signSubmitAndWatch).toHaveBeenCalledTimes(2);
    });

    it('fails after retry is also exhausted', async () => {
      const signSubmitAndWatch = mockErrorObservable(new Error('Stale'));
      const mockApi = createMockApi(signSubmitAndWatch);
      const lazyClient = { getClient: () => ({ getUnsafeApi: () => mockApi }) } as any;
    ...
    it('rejects on finalized dispatch error', async () => {
      const signSubmitAndWatch = vi.fn(() => ({
        subscribe(observer: { next?: (event: any) => void; error?: (err: any) => void }) {
          queueMicrotask(() =>
            observer.next?.({
              type: 'finalized',
              ok: false,
              dispatchError: { type: 'Module', value: { type: 'PeopleLite', value: { type: 'AlreadyRegistered' } } },
            }),
          );
          return { unsubscribe: vi.fn() };
        },
      ...
      // Dispatch errors are rejected but retried (since withRetry catches all errors)
      // After 2 attempts (1 + 1 retry), it should fail
      expect(result.isErr()).toBe(true);
      expect(signSubmitAndWatch).toHaveBeenCalledTimes(2);
    });
  });
