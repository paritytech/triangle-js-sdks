...
describe('createSandbox', () => {
  ...
  describe('run', () => {
    it('should evaluate code without throwing', async () => {
      const sandbox = await createSandbox();
      expect(() => sandbox.run('const x = 1 + 2')).not.toThrow();
      sandbox.dispose();
    });
    ...
    it('should run async IIFE without throwing', async () => {
      const sandbox = await createSandbox();
      expect(() => sandbox.run('(async () => { await Promise.resolve(); })()')).not.toThrow();
      sandbox.dispose();
    });
    ...
    it('should deliver data posted inside an async IIFE', async () => {
      ...
      sandbox.run('(async () => { await Promise.resolve(); __HOST_API_PORT__.postMessage(new Uint8Array([7])); })()');
  ...
  describe('port messaging', () => {
    it('should call provider subscriber when sandbox calls port.postMessage', async () => {
      ...
      sandbox.run('__HOST_API_PORT__.postMessage(new Uint8Array([1, 2, 3]))');
    ...
    it('should clear the subscriber when the returned unsubscribe is called', async () => {
      ...
      unsubscribe();
      sandbox.run('__HOST_API_PORT__.postMessage(new Uint8Array([1]))');
    ...
    it('should read and set port.onmessage via the getter', async () => {
      ...
      expect(() => sandbox.run('')).not.toThrow();
      sandbox.dispose();
    });
  });
