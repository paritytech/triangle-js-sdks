...
fn host_chat_list_subscribe(callback: fn(Vector<ChatRoom>)) -> Result<Subscriber, GenericErr>;
...
The returned `Subscriber` interface depends on the implementation, but a generic interface can look like this:
```rust
struct Subscriber {
  unsubscribe: fn(),
  onInterrupt: fn(fn())
